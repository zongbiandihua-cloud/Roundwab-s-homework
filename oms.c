...
/* 比较键：先比学号，再比课程编号（AVL 树排序用） */
static int key_cmp(const CourseRecord *r, const char *sid, const char *cid) {
    int c = strcmp(r->studentId, sid);
    if (c != 0) return c;
    return strcmp(r->courseId, cid);
}
